...
class TradeProposalsDeclined : Fragment(), TradeProposalViewAdapterR.ClickListener {

    private lateinit var tradeProposalViewAdapterR: TradeProposalViewAdapterR
    private lateinit var emptyListText: TextView
    private lateinit var declinedProgressBar: ProgressBar

    private var userID: Int = 0
    private var declinedListOfTradeProposals = mutableListOf<TradeProposalsRResultBody>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            tradeProposalsVM.getTradeProposals(userID, 71)
        }
    }
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.f_r_trade_proposals_declined, container, false)

        emptyListText = view.findViewById(R.id.f_r_tp_empty_declined_list_text)
        declinedProgressBar = view.findViewById(R.id.declined_pb)

        tradeProposalsVM.tpmld.observe(viewLifecycleOwner) { response ->
            declinedListOfTradeProposals = response as MutableList<TradeProposalsRResultBody>

            emptyListText.apply {
                if (declinedListOfTradeProposals.size == 0){
                    this.visibility = View.VISIBLE
                }
            }

            initRecyclerView(view)
            declinedProgressBar.visibility = View.GONE
        }
        return view
    }

    private fun initRecyclerView(view: View){
        val recyclerView = view.findViewById<RecyclerView>(R.id.f_r_tp_declined_list)
        recyclerView.layoutManager = LinearLayoutManager(activity)
        tradeProposalViewAdapterR = TradeProposalViewAdapterR(
            declinedListOfTradeProposals,
            "#ffE91E63",
            this
        )
